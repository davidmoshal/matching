...
Please note the `Either.left()` ,which is the exception with any event, is only used when
* The State is not found for update and delete commands
* The State already exists for create command
* The command has failed validation, there is no state change, and there is no intention to even generate a rejected event. However, in most cases it is preferable to generate a rejected event for audit purpose, plus there may be cases in which rejection could change state. E.g. Cancel existing quotes when rejecting a new mass quote.  
