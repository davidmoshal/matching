...
internal class `Given the book has a SELL Limit GTC Order 4 at 10` : FeatureSpec({
    ...
    feature(noTradeIfPricesDoNotCrossFeature) {
        scenario(noTradeIfPricesDoNotCrossFeature + "When a BUY Limit GTC Order 2 at 9 is placed, then the new entry is added to the BUY side") {
            val orderPlacedEvent = anOrderPlacedEvent(
                bookId = bookId,
                entryType = EntryType.LIMIT,
                side = Side.BUY,
    ...
    feature(noWashTradeFeature) {
        scenario(noWashTradeFeature + "When a BUY Limit GTC Order 4 at 10 is placed by the same firm and same firm client, then the new entry is added to the BUY side") {
            val orderPlacedEvent = anOrderPlacedEvent(
                requestId = anotherClientRequestId(),
                whoRequested = anotherFirmWithClient(),
                bookId = bookId,
        ...
        scenario(noWashTradeFeature + "When a BUY Limit GTC Order 4 at 10 is placed by the same firm, one with but another without firm client, then the new entry is added to the BUY side") {
            val orderPlacedEvent = anOrderPlacedEvent(
                requestId = anotherClientRequestId(),
        ...
        scenario(noWashTradeFeature + "When a BUY Limit GTC Order 4 at 10 is placed by the same firm, both without firm client, then the new entry is added to the BUY side") {
            val orderPlacedEvent = anOrderPlacedEvent(
                requestId = anotherClientRequestId(),
                whoRequested = anotherFirmWithoutClient(),
                bookId = bookId,
